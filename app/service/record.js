...
class RecordService extends Service {
  ...
  async findReocrdById(id) {
    try {
      const record = await this.ctx.model.Record.findOne({ where: { id } });
      this.ctx.status = 200;
  ...
  async getRecordTotal() {
    try {
      const res = await this.service.book.getBooks(1, 10, true);
  ...
  async addRecord(model) {
    const res = this.ctx.model.Record.create(model);
